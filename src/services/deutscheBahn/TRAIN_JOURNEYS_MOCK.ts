const STATION_NAMES = [
  "Aachen Hbf",
  "Aalen Hbf",
  "Ahlen (Westf)",
  "Alexanderplatz",
  "Altenbeken",
  "Althof",
  "Andernach",
  "Angermünde",
  "Anklam",
  "Ansbach",
  "Aschaffenburg Hbf",
  "Aschau (Chiemgau)",
  "Aschersleben Pbf",
  "Bad Homburg",
  "Bad Kleinen",
  "Bad Kreuznach",
  "Bad Oeynhausen",
  "Bad Oldesloe",
  "Bad Reichenhall",
  "Bad Sassendorf",
  "Baden-Baden",
  "Bamberg",
  "Bayreuth Hbf",
  "Bergen auf Rügen",
  "Berlin-Friedrichstraße",
  "Berlin Gesundbrunnen",
  "Berlin Ostbahnhof",
  "Berlin Zoologischer Garten",
  "Berlin-Lichtenberg",
  "Berlin-Spandau",
  "Bernau (b Berlin)",
  "Bielefeld Hbf",
  "Bingen (Rhein) Hbf",
  "Ostseebad Binz",
  "Bochum Hbf",
  "Bonn Hbf",
  "Braunschweig Hbf",
  "Bremen Hbf",
  "Bremerhaven Hbf",
  "Broderstorf",
  "Bruchsal",
  "Büchen",
  "Buchloe",
  "Bützow",
  "Chemnitz Hbf",
  "Coburg",
  "Berlin Hauptbahnhof",
  "Cottbus Hbf",
  "Darmstadt Hbf",
  "Delmenhorst",
  "Dessau Hbf",
  "Doberlug-Kirchhain",
  "Donauwörth",
  "Dresden Hbf",
  "Dresden-Neustadt",
  "Duisburg Hbf",
  "Düsseldorf Flughafen",
  "Düsseldorf Hbf",
  "Eberswalde Hbf",
  "Eckernförde",
  "Eisenach",
  "Elmshorn",
  "Emden Hbf",
  "Erfurt Hbf",
  "Erlangen",
  "Essen Hbf",
  "Finsterwalde (Niederlausitz)",
  "Fischen",
  "Flensburg",
  "Flughafen München",
  "Forst (Lausitz)",
  "Frankfurt (Main) Flughafen Regionalbahnhof",
  "Frankfurt am Main Stadion",
  "Frankfurt (Oder)",
  "Frankfurt (Main) Hbf",
  "Frankfurt (Main) Niederrad",
  "Freiburg (Breisgau) Hbf",
  "Fulda",
  "Füssen",
  "Garmisch-Partenkirchen",
  "Gelsenkirchen Hbf",
  "Gera Hbf",
  "Gießen",
  "Göttingen",
  "Greifswald",
  "Grevesmühlen",
  "Groß Lüsewitz",
  "Groß Schwaß",
  "Güstrow",
  "Gütersloh Hbf",
  "Hagebök",
  "Hagen Hbf",
  "Halberstadt",
  "Halle (Saale) Hbf",
  "Hamburg Dammtor",
  "Hamburg Hbf",
  "Hamburg-Altona",
  "Hamburg-Bergedorf",
  "Hamburg-Harburg",
  "Hameln",
  "Hamm (Westf) Hbf",
  "Hannover Hbf",
  "Heide (Holst)",
  "Heidelberg Hbf",
  "Heilbronn Hbf",
  "Herford",
  "Hildesheim Hbf",
  "Hof Hbf",
  "Homburg (Saar) Hbf",
  "Hornstorf",
  "Husum",
  "Ingolstadt Hbf",
  "Jena Paradies",
  "Jena West",
  "Kaiserslautern Hbf",
  "Kamen",
  "Karlsruhe Hbf",
  "Kassel-Wilhelmshöhe",
  "Kempten (Allgäu) Hbf",
  "Kiel Hbf",
  "Koblenz Hbf",
  "Köln Hbf",
  "Konstanz",
  "Kröpelin",
  "Hannover-Messe / Laatzen",
  "Leer (Ostfriesl)",
  "Leipzig Hbf",
  "Lichtenfels",
  "Limburg (Lahn)",
  "Limburg Süd",
  "Lindau-Insel",
  "Lindau-Reutin",
  "Lippstadt",
  "Ludwigshafen (Rhein) Hbf",
  "Lüneburg",
  "Lüssow (Meckl)",
  "Lutherstadt Wittenberg Hauptbahnhof",
  "Magdeburg Hbf",
  "Mainz Hbf",
  "Bad Malente-Gremsmühlen",
  "Mannheim Hbf",
  "Marburg (Lahn)",
  "Memmingen",
  "Minden (Westf)",
  "Mönchengladbach Hbf",
  "Montabaur",
  "Mülheim (Ruhr) Hbf",
  "München Hbf",
  "München Ost",
  "Münster (Westf) Hbf",
  "Murnau",
  "Naumburg (Saale) Hbf",
  "Neubrandenburg",
  "Neubukow",
  "Neumünster",
  "Neuss Hbf",
  "Neustadt (Holst)",
  "Neustadt (Weinstr) Hbf",
  "Neuwied",
  "Niebüll",
  "Nienburg (Weser)",
  "Norddeich Mole",
  "Northeim (Han)",
  "Nürnberg Hbf",
  "Oberhausen Hbf",
  "Oberstdorf",
  "Offenburg",
  "Oldenburg (Oldb) Hbf",
  "Osnabrück Hbf",
  "Berlin Ostkreuz",
  "Graal-Müritz",
  "Paderborn Hbf",
  "Berlin Südkreuz",
  "Parkentin",
  "Pasewalk",
  "Passau Hbf",
  "Pforzheim Hbf",
  "Plattling",
  "Plauen (Vogtl) ob Bf",
  "Berlin Potsdamer Platz",
  "Prien am Chiemsee",
  "Quedlinburg",
  "Reddelich",
  "Regensburg Hbf",
  "Remagen",
  "Rendsburg",
  "Rheda-Wiedenbrück",
  "Rheine",
  "Ribnitz-Damgarten Ost",
  "Roggentin",
  "Rosenheim",
  "Rostock Hbf",
  "Rotenburg (Wümme)",
  "Röthenbach (Allgäu)",
  "Saalfeld (Saale)",
  "Saarbrücken Hbf",
  "Saarlouis Hbf",
  "Sandhagen (b Bad Doberan)",
  "Sanitz (b Rostock)",
  "Schleswig",
  "Schwäbisch Gmünd",
  "Schweinfurt Hbf",
  "Schwerin Hbf",
  "Siegburg/Bonn",
  "Siegen Hbf",
  "Sierksdorf",
  "Singen (Hohentwiel)",
  "Soest",
  "Solingen Hbf",
  "Steinhausen-Neuburg",
  "Stendal Hbf",
  "Stralsund Hbf",
  "Straubing",
  "Stuttgart Hbf",
  "Teschow",
  "Tessin",
  "Thale Hbf",
  "Timmendorferstrand",
  "Traunstein",
  "Treuchtlingen",
  "Trier Hbf",
  "Tübingen Hbf",
  "Ueckermünde",
  "Uelzen",
  "Ulm Hbf",
  "Verden (Aller)",
  "Warnemünde",
  "Weimar",
  "Wernigerode Hbf",
  "Westerland (Sylt)",
  "Wiesbaden Hbf",
  "Wismar",
  "Wittlich Hbf",
  "Wolfsburg Hbf",
  "Worms Hbf",
  "Wuppertal Hbf",
  "Würzburg Hbf",
  "Züssow",
  "Zwickau (Sachs) Hbf",
  "Langenhagen-Mitte",
  "Frankfurt am Main Flughafen Fernbahnhof",
  "Tessin West",
  "Rostock Thierfelder Straße",
  "Kalsow",
  "Flughafen Leipzig/Halle",
  "Coburg Nord",
  "Flughafen BER - Terminal 1-2",
] as const;

interface Stop {
  station: (typeof STATION_NAMES)[number];
  track: string;
  arrivalTime: string;
  departureTime: string;
  stationId: string;
  evaNumber: number;
}

interface TrainJourney {
  departure: Stop;
  destination: Stop;
  train_type: "ICE" | "EC" | "IC" | "RE" | "RB" | "S" | "U";
  train_number: string;
  route: Stop[];
}

export const TRAIN_JOURNEYS_MOCK: TrainJourney[] = [
  {
    train_type: "ICE",
    train_number: "ICE 123",
    destination: {
      station: "Hamburg Hbf",
      track: "6",
      arrivalTime: "11:01",
      departureTime: "11:05",
      stationId: "2514",
      evaNumber: 8002549,
    },
    departure: {
      station: "Berlin Gesundbrunnen",
      track: "2",
      arrivalTime: "08:00",
      departureTime: "08:03",
      stationId: "528",
      evaNumber: 8011102,
    },
    route: [
      {
        station: "Berlin Gesundbrunnen",
        track: "2",
        arrivalTime: "08:00",
        departureTime: "08:03",
        stationId: "528",
        evaNumber: 8011102,
      },
      {
        station: "Hannover Hbf",
        track: "5",
        arrivalTime: "09:00",
        departureTime: "09:20",
        stationId: "2545",
        evaNumber: 8000152,
      },
      {
        station: "Hamburg Hbf",
        track: "6",
        arrivalTime: "11:01",
        departureTime: "11:05",
        stationId: "2514",
        evaNumber: 8002549,
      },
    ],
  },
  {
    train_type: "IC",
    train_number: "IC 456",
    destination: {
      station: "Frankfurt (Main) Hbf",
      track: "4",
      arrivalTime: "12:45",
      departureTime: "12:52",
      stationId: "1866",
      evaNumber: 8000105,
    },
    departure: {
      station: "München Hbf",
      track: "7",
      arrivalTime: "09:30",
      departureTime: "09:35",
      stationId: "4234",
      evaNumber: 8000261,
    },
    route: [
      {
        station: "München Hbf",
        track: "7",
        arrivalTime: "09:30",
        departureTime: "09:35",
        stationId: "4234",
        evaNumber: 8000261,
      },
      {
        station: "Nürnberg Hbf",
        track: "6",
        arrivalTime: "10:15",
        departureTime: "10:20",
        stationId: "4593",
        evaNumber: 8000284,
      },
      {
        station: "Frankfurt (Main) Hbf",
        track: "4",
        arrivalTime: "12:45",
        departureTime: "12:52",
        stationId: "1866",
        evaNumber: 8000105,
      },
    ],
  },
  {
    train_type: "EC",
    train_number: "EC 789",
    destination: {
      station: "Köln Hbf",
      track: "8",
      arrivalTime: "14:20",
      departureTime: "14:25",
      stationId: "3320",
      evaNumber: 8000207,
    },
    departure: {
      station: "Hamburg Hbf",
      track: "3",
      arrivalTime: "10:45",
      departureTime: "10:50",
      stationId: "2514",
      evaNumber: 8002549,
    },
    route: [
      {
        station: "Hamburg Hbf",
        track: "3",
        arrivalTime: "10:45",
        departureTime: "10:50",
        stationId: "2514",
        evaNumber: 8002549,
      },
      {
        station: "Bremen Hbf",
        track: "6",
        arrivalTime: "11:30",
        departureTime: "11:35",
        stationId: "855",
        evaNumber: 8000050,
      },
      {
        station: "Köln Hbf",
        track: "8",
        arrivalTime: "14:20",
        departureTime: "14:25",
        stationId: "3320",
        evaNumber: 8000207,
      },
    ],
  },
  {
    train_type: "RE",
    train_number: "RE 234",
    destination: {
      station: "Stuttgart Hbf",
      track: "5",
      arrivalTime: "15:30",
      departureTime: "15:35",
      stationId: "6071",
      evaNumber: 8000096,
    },
    departure: {
      station: "Frankfurt (Main) Hbf",
      track: "1",
      arrivalTime: "12:00",
      departureTime: "12:05",
      stationId: "1866",
      evaNumber: 8000105,
    },
    route: [
      {
        station: "Frankfurt (Main) Hbf",
        track: "1",
        arrivalTime: "12:00",
        departureTime: "12:05",
        stationId: "1866",
        evaNumber: 8000105,
      },
      {
        station: "Mannheim Hbf",
        track: "4",
        arrivalTime: "13:00",
        departureTime: "13:05",
        stationId: "3925",
        evaNumber: 8000244,
      },
      {
        station: "Stuttgart Hbf",
        track: "5",
        arrivalTime: "15:30",
        departureTime: "15:35",
        stationId: "6071",
        evaNumber: 8000096,
      },
    ],
  },
  {
    train_type: "IC",
    train_number: "IC 567",
    destination: {
      station: "Dresden Hbf",
      track: "2",
      arrivalTime: "16:15",
      departureTime: "16:20",
      stationId: "1343",
      evaNumber: 8010085,
    },
    departure: {
      station: "Berlin Gesundbrunnen",
      track: "3",
      arrivalTime: "13:30",
      departureTime: "13:35",
      stationId: "528",
      evaNumber: 8011102,
    },
    route: [
      {
        station: "Berlin Gesundbrunnen",
        track: "3",
        arrivalTime: "13:30",
        departureTime: "13:35",
        stationId: "528",
        evaNumber: 8011102,
      },
      {
        station: "Leipzig Hbf",
        track: "7",
        arrivalTime: "14:45",
        departureTime: "14:50",
        stationId: "3631",
        evaNumber: 8010205,
      },
      {
        station: "Dresden Hbf",
        track: "2",
        arrivalTime: "16:15",
        departureTime: "16:20",
        stationId: "1343",
        evaNumber: 8010085,
      },
    ],
  },
  {
    train_type: "RE",
    train_number: "RE 890",
    destination: {
      station: "Berlin Gesundbrunnen",
      track: "5",
      arrivalTime: "18:40",
      departureTime: "18:45",
      stationId: "528",
      evaNumber: 8011102,
    },
    departure: {
      station: "Hamburg Hbf",
      track: "2",
      arrivalTime: "15:15",
      departureTime: "15:20",
      stationId: "2514",
      evaNumber: 8002549,
    },
    route: [
      {
        station: "Hamburg Hbf",
        track: "2",
        arrivalTime: "15:15",
        departureTime: "15:20",
        stationId: "2514",
        evaNumber: 8002549,
      },
      {
        station: "Schwerin Hbf",
        track: "4",
        arrivalTime: "16:05",
        departureTime: "16:10",
        stationId: "5755",
        evaNumber: 8010324,
      },
      {
        station: "Berlin Gesundbrunnen",
        track: "5",
        arrivalTime: "18:40",
        departureTime: "18:45",
        stationId: "528",
        evaNumber: 8011102,
      },
    ],
  },
  {
    train_type: "RE",
    train_number: "RE 456",
    destination: {
      station: "Köln Hbf",
      track: "2",
      arrivalTime: "20:45",
      departureTime: "20:50",
      stationId: "3320",
      evaNumber: 8000207,
    },
    departure: {
      station: "Frankfurt (Main) Hbf",
      track: "7",
      arrivalTime: "18:15",
      departureTime: "18:20",
      stationId: "1866",
      evaNumber: 8000105,
    },
    route: [
      {
        station: "Frankfurt (Main) Hbf",
        track: "7",
        arrivalTime: "18:15",
        departureTime: "18:20",
        stationId: "1866",
        evaNumber: 8000105,
      },
      {
        station: "Koblenz Hbf",
        track: "4",
        arrivalTime: "19:45",
        departureTime: "19:50",
        stationId: "3299",
        evaNumber: 8000206,
      },
      {
        station: "Köln Hbf",
        track: "2",
        arrivalTime: "20:45",
        departureTime: "20:50",
        stationId: "3320",
        evaNumber: 8000207,
      },
    ],
  },
  {
    train_type: "ICE",
    train_number: "ICE 789",
    destination: {
      station: "Hamburg Hbf",
      track: "3",
      arrivalTime: "22:55",
      departureTime: "23:00",
      stationId: "2514",
      evaNumber: 8002549,
    },
    departure: {
      station: "Berlin Gesundbrunnen",
      track: "5",
      arrivalTime: "19:30",
      departureTime: "19:35",
      stationId: "528",
      evaNumber: 8011102,
    },
    route: [
      {
        station: "Berlin Gesundbrunnen",
        track: "5",
        arrivalTime: "19:30",
        departureTime: "19:35",
        stationId: "528",
        evaNumber: 8011102,
      },
      {
        station: "Hannover Hbf",
        track: "3",
        arrivalTime: "21:00",
        departureTime: "21:05",
        stationId: "2545",
        evaNumber: 8000152,
      },
      {
        station: "Hamburg Hbf",
        track: "3",
        arrivalTime: "22:55",
        departureTime: "23:00",
        stationId: "2514",
        evaNumber: 8002549,
      },
    ],
  },
  {
    train_type: "ICE",
    train_number: "ICE 123",
    destination: {
      station: "Hamburg Hbf",
      track: "6",
      arrivalTime: "11:01",
      departureTime: "11:05",
      stationId: "2514",
      evaNumber: 8002549,
    },
    departure: {
      station: "München Hbf",
      track: "3",
      arrivalTime: "08:00",
      departureTime: "08:03",
      stationId: "4234",
      evaNumber: 8000261,
    },
    route: [
      {
        station: "München Hbf",
        track: "3",
        arrivalTime: "08:00",
        departureTime: "08:03",
        stationId: "4234",
        evaNumber: 8000261,
      },
      {
        station: "Nürnberg Hbf",
        track: "5",
        arrivalTime: "09:00",
        departureTime: "09:20",
        stationId: "4593",
        evaNumber: 8000284,
      },
      {
        station: "Hamburg Hbf",
        track: "6",
        arrivalTime: "11:01",
        departureTime: "11:05",
        stationId: "2514",
        evaNumber: 8002549,
      },
    ],
  },
  {
    train_type: "IC",
    train_number: "IC 456",
    destination: {
      station: "Frankfurt (Main) Hbf",
      track: "4",
      arrivalTime: "12:45",
      departureTime: "12:52",
      stationId: "1866",
      evaNumber: 8000105,
    },
    departure: {
      station: "Berlin Gesundbrunnen",
      track: "2",
      arrivalTime: "09:30",
      departureTime: "09:35",
      stationId: "528",
      evaNumber: 8011102,
    },
    route: [
      {
        station: "Berlin Gesundbrunnen",
        track: "2",
        arrivalTime: "09:30",
        departureTime: "09:35",
        stationId: "528",
        evaNumber: 8011102,
      },
      {
        station: "Hannover Hbf",
        track: "5",
        arrivalTime: "10:20",
        departureTime: "10:25",
        stationId: "2545",
        evaNumber: 8000152,
      },
      {
        station: "Frankfurt (Main) Hbf",
        track: "4",
        arrivalTime: "12:45",
        departureTime: "12:52",
        stationId: "1866",
        evaNumber: 8000105,
      },
    ],
  },
  {
    train_type: "EC",
    train_number: "EC 789",
    destination: {
      station: "Köln Hbf",
      track: "8",
      arrivalTime: "14:20",
      departureTime: "14:25",
      stationId: "3320",
      evaNumber: 8000207,
    },
    departure: {
      station: "Hamburg Hbf",
      track: "3",
      arrivalTime: "10:45",
      departureTime: "10:50",
      stationId: "2514",
      evaNumber: 8002549,
    },
    route: [
      {
        station: "Hamburg Hbf",
        track: "3",
        arrivalTime: "10:45",
        departureTime: "10:50",
        stationId: "2514",
        evaNumber: 8002549,
      },
      {
        station: "Bremen Hbf",
        track: "6",
        arrivalTime: "11:30",
        departureTime: "11:35",
        stationId: "855",
        evaNumber: 8000050,
      },
      {
        station: "Köln Hbf",
        track: "8",
        arrivalTime: "14:20",
        departureTime: "14:25",
        stationId: "3320",
        evaNumber: 8000207,
      },
    ],
  },
  {
    train_type: "RE",
    train_number: "RE 234",
    destination: {
      station: "Stuttgart Hbf",
      track: "5",
      arrivalTime: "15:30",
      departureTime: "15:35",
      stationId: "6071",
      evaNumber: 8000096,
    },
    departure: {
      station: "Frankfurt (Main) Hbf",
      track: "1",
      arrivalTime: "12:00",
      departureTime: "12:05",
      stationId: "1866",
      evaNumber: 8000105,
    },
    route: [
      {
        station: "Frankfurt (Main) Hbf",
        track: "1",
        arrivalTime: "12:00",
        departureTime: "12:05",
        stationId: "1866",
        evaNumber: 8000105,
      },
      {
        station: "Heidelberg Hbf",
        track: "3",
        arrivalTime: "13:00",
        departureTime: "13:05",
        stationId: "2628",
        evaNumber: 8000156,
      },
      {
        station: "Stuttgart Hbf",
        track: "5",
        arrivalTime: "15:30",
        departureTime: "15:35",
        stationId: "6071",
        evaNumber: 8000096,
      },
    ],
  },
  {
    train_type: "EC",
    train_number: "EC 890",
    destination: {
      station: "Rosenheim",
      track: "3",
      arrivalTime: "16:20",
      departureTime: "16:25",
      stationId: "5348",
      evaNumber: 8000320,
    },
    departure: {
      station: "München Hbf",
      track: "6",
      arrivalTime: "14:30",
      departureTime: "14:35",
      stationId: "4234",
      evaNumber: 8000261,
    },
    route: [
      {
        station: "München Hbf",
        track: "6",
        arrivalTime: "14:30",
        departureTime: "14:35",
        stationId: "4234",
        evaNumber: 8000261,
      },
      {
        station: "Rosenheim",
        track: "4",
        arrivalTime: "15:20",
        departureTime: "15:25",
        stationId: "5348",
        evaNumber: 8000320,
      },
    ],
  },
  {
    train_type: "ICE",
    train_number: "ICE 456",
    destination: {
      station: "Leipzig Hbf",
      track: "7",
      arrivalTime: "16:25",
      departureTime: "16:30",
      stationId: "3631",
      evaNumber: 8010205,
    },
    departure: {
      station: "Berlin Gesundbrunnen",
      track: "1",
      arrivalTime: "15:00",
      departureTime: "15:05",
      stationId: "528",
      evaNumber: 8011102,
    },
    route: [
      {
        station: "Berlin Gesundbrunnen",
        track: "1",
        arrivalTime: "15:00",
        departureTime: "15:05",
        stationId: "528",
        evaNumber: 8011102,
      },
      {
        station: "Halle (Saale) Hbf",
        track: "4",
        arrivalTime: "15:45",
        departureTime: "15:50",
        stationId: "2498",
        evaNumber: 8010159,
      },
      {
        station: "Leipzig Hbf",
        track: "7",
        arrivalTime: "16:25",
        departureTime: "16:30",
        stationId: "3631",
        evaNumber: 8010205,
      },
    ],
  },
  {
    train_type: "IC",
    train_number: "IC 789",
    destination: {
      station: "Flensburg",
      track: "3",
      arrivalTime: "17:45",
      departureTime: "17:50",
      stationId: "1810",
      evaNumber: 8000103,
    },
    departure: {
      station: "Hamburg Hbf",
      track: "8",
      arrivalTime: "16:15",
      departureTime: "16:20",
      stationId: "2514",
      evaNumber: 8002549,
    },
    route: [
      {
        station: "Hamburg Hbf",
        track: "8",
        arrivalTime: "16:15",
        departureTime: "16:20",
        stationId: "2514",
        evaNumber: 8002549,
      },
      {
        station: "Flensburg",
        track: "3",
        arrivalTime: "17:45",
        departureTime: "17:50",
        stationId: "1810",
        evaNumber: 8000103,
      },
    ],
  },
  {
    train_type: "ICE",
    train_number: "ICE 567",
    destination: {
      station: "Mannheim Hbf",
      track: "5",
      arrivalTime: "18:15",
      departureTime: "18:20",
      stationId: "3925",
      evaNumber: 8000244,
    },
    departure: {
      station: "Frankfurt (Main) Hbf",
      track: "3",
      arrivalTime: "17:30",
      departureTime: "17:35",
      stationId: "1866",
      evaNumber: 8000105,
    },
    route: [
      {
        station: "Frankfurt (Main) Hbf",
        track: "3",
        arrivalTime: "17:30",
        departureTime: "17:35",
        stationId: "1866",
        evaNumber: 8000105,
      },
      {
        station: "Mannheim Hbf",
        track: "5",
        arrivalTime: "18:15",
        departureTime: "18:20",
        stationId: "3925",
        evaNumber: 8000244,
      },
    ],
  },
  {
    train_type: "RE",
    train_number: "RE 123",
    destination: {
      station: "Düsseldorf Hbf",
      track: "7",
      arrivalTime: "19:30",
      departureTime: "19:35",
      stationId: "1401",
      evaNumber: 8000085,
    },
    departure: {
      station: "Bonn Hbf",
      track: "4",
      arrivalTime: "18:45",
      departureTime: "18:50",
      stationId: "767",
      evaNumber: 8000044,
    },
    route: [
      {
        station: "Bonn Hbf",
        track: "4",
        arrivalTime: "18:45",
        departureTime: "18:50",
        stationId: "767",
        evaNumber: 8000044,
      },
      {
        station: "Köln Hbf",
        track: "2",
        arrivalTime: "19:10",
        departureTime: "19:15",
        stationId: "3320",
        evaNumber: 8000207,
      },
      {
        station: "Düsseldorf Hbf",
        track: "7",
        arrivalTime: "19:30",
        departureTime: "19:35",
        stationId: "1401",
        evaNumber: 8000085,
      },
    ],
  },
  {
    train_type: "IC",
    train_number: "IC 456",
    destination: {
      station: "Stendal Hbf",
      track: "2",
      arrivalTime: "14:45",
      departureTime: "14:50",
      stationId: "6010",
      evaNumber: 8010334,
    },
    departure: {
      station: "Trier Hbf",
      track: "1",
      arrivalTime: "10:15",
      departureTime: "10:20",
      stationId: "6264",
      evaNumber: 8000134,
    },
    route: [
      {
        station: "Trier Hbf",
        track: "1",
        arrivalTime: "10:15",
        departureTime: "10:20",
        stationId: "6264",
        evaNumber: 8000134,
      },
      {
        station: "Koblenz Hbf",
        track: "2",
        arrivalTime: "11:30",
        departureTime: "11:35",
        stationId: "3299",
        evaNumber: 8000206,
      },
      {
        station: "Bonn Hbf",
        track: "4",
        arrivalTime: "12:15",
        departureTime: "12:20",
        stationId: "767",
        evaNumber: 8000044,
      },
      {
        station: "Stendal Hbf",
        track: "2",
        arrivalTime: "14:45",
        departureTime: "14:50",
        stationId: "6010",
        evaNumber: 8010334,
      },
    ],
  },
  {
    train_type: "RB",
    train_number: "RB 234",
    destination: {
      station: "Schweinfurt Hbf",
      track: "3",
      arrivalTime: "11:15",
      departureTime: "11:20",
      stationId: "5742",
      evaNumber: 8000032,
    },
    departure: {
      station: "Treuchtlingen",
      track: "2",
      arrivalTime: "09:00",
      departureTime: "09:05",
      stationId: "6252",
      evaNumber: 8000122,
    },
    route: [
      {
        station: "Treuchtlingen",
        track: "2",
        arrivalTime: "09:00",
        departureTime: "09:05",
        stationId: "6252",
        evaNumber: 8000122,
      },
      {
        station: "Donauwörth",
        track: "1",
        arrivalTime: "09:40",
        departureTime: "09:45",
        stationId: "1265",
        evaNumber: 8000078,
      },
      {
        station: "Würzburg Hbf",
        track: "5",
        arrivalTime: "10:45",
        departureTime: "10:50",
        stationId: "6945",
        evaNumber: 8000260,
      },
      {
        station: "Schweinfurt Hbf",
        track: "3",
        arrivalTime: "11:15",
        departureTime: "11:20",
        stationId: "5742",
        evaNumber: 8000032,
      },
    ],
  },
  {
    train_type: "RE",
    train_number: "RE 789",
    destination: {
      station: "Bonn Hbf",
      track: "5",
      arrivalTime: "10:15",
      departureTime: "10:20",
      stationId: "767",
      evaNumber: 8000044,
    },
    departure: {
      station: "Aachen Hbf",
      track: "3",
      arrivalTime: "08:30",
      departureTime: "08:35",
      stationId: "1",
      evaNumber: 8000001,
    },
    route: [
      {
        station: "Aachen Hbf",
        track: "3",
        arrivalTime: "08:30",
        departureTime: "08:35",
        stationId: "1",
        evaNumber: 8000001,
      },
      {
        station: "Köln Hbf",
        track: "8",
        arrivalTime: "09:45",
        departureTime: "09:50",
        stationId: "3320",
        evaNumber: 8000207,
      },
      {
        station: "Bonn Hbf",
        track: "5",
        arrivalTime: "10:15",
        departureTime: "10:20",
        stationId: "767",
        evaNumber: 8000044,
      },
    ],
  },
  {
    train_type: "IC",
    train_number: "IC 567",
    destination: {
      station: "Berlin Gesundbrunnen",
      track: "7",
      arrivalTime: "14:00",
      departureTime: "14:05",
      stationId: "528",
      evaNumber: 8011102,
    },
    departure: {
      station: "Magdeburg Hbf",
      track: "4",
      arrivalTime: "11:30",
      departureTime: "11:35",
      stationId: "3881",
      evaNumber: 8010224,
    },
    route: [
      {
        station: "Magdeburg Hbf",
        track: "4",
        arrivalTime: "11:30",
        departureTime: "11:35",
        stationId: "3881",
        evaNumber: 8010224,
      },
      {
        station: "Halle (Saale) Hbf",
        track: "3",
        arrivalTime: "12:30",
        departureTime: "12:35",
        stationId: "2498",
        evaNumber: 8010159,
      },
      {
        station: "Berlin Gesundbrunnen",
        track: "7",
        arrivalTime: "14:00",
        departureTime: "14:05",
        stationId: "528",
        evaNumber: 8011102,
      },
    ],
  },
  {
    train_type: "IC",
    train_number: "IC 123",
    destination: {
      station: "Würzburg Hbf",
      track: "5",
      arrivalTime: "12:45",
      departureTime: "12:50",
      stationId: "6945",
      evaNumber: 8000260,
    },
    departure: {
      station: "Rosenheim",
      track: "2",
      arrivalTime: "09:15",
      departureTime: "09:20",
      stationId: "5348",
      evaNumber: 8000320,
    },
    route: [
      {
        station: "Rosenheim",
        track: "2",
        arrivalTime: "09:15",
        departureTime: "09:20",
        stationId: "5348",
        evaNumber: 8000320,
      },
      {
        station: "München Hbf",
        track: "7",
        arrivalTime: "10:15",
        departureTime: "10:20",
        stationId: "4234",
        evaNumber: 8000261,
      },
      {
        station: "Nürnberg Hbf",
        track: "3",
        arrivalTime: "11:45",
        departureTime: "11:50",
        stationId: "4593",
        evaNumber: 8000284,
      },
      {
        station: "Würzburg Hbf",
        track: "5",
        arrivalTime: "12:45",
        departureTime: "12:50",
        stationId: "6945",
        evaNumber: 8000260,
      },
    ],
  },
  {
    train_type: "ICE",
    train_number: "ICE 789",
    destination: {
      station: "München Hbf",
      track: "10",
      arrivalTime: "13:30",
      departureTime: "13:35",
      stationId: "4234",
      evaNumber: 8000261,
    },
    departure: {
      station: "Münster (Westf) Hbf",
      track: "5",
      arrivalTime: "08:00",
      departureTime: "08:05",
      stationId: "4280",
      evaNumber: 8000263,
    },
    route: [
      {
        station: "Münster (Westf) Hbf",
        track: "5",
        arrivalTime: "08:00",
        departureTime: "08:05",
        stationId: "4280",
        evaNumber: 8000263,
      },
      {
        station: "Delmenhorst",
        track: "3",
        arrivalTime: "08:45",
        departureTime: "08:50",
        stationId: "1159",
        evaNumber: 8000070,
      },
      {
        station: "Köln Hbf",
        track: "7",
        arrivalTime: "09:45",
        departureTime: "09:50",
        stationId: "3320",
        evaNumber: 8000207,
      },
      {
        station: "München Hbf",
        track: "10",
        arrivalTime: "13:30",
        departureTime: "13:35",
        stationId: "4234",
        evaNumber: 8000261,
      },
    ],
  },
];
